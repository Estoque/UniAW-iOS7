...
var NUMBER_OF_FAIR_CLOUDS = 6; // fair clouds to show on screen
var NUMBER_OF_FAIR_CLOUD_IMAGES = 7; // images + 1
var container4 = document.getElementById("cloudContainer");
for (var i = 0; i < NUMBER_OF_FAIR_CLOUDS; i++) { container4.appendChild(createACloud()); }
...
if (where == "day") {
	...
	if (V_Formation_Birds == true) {
			...
			container5.appendChild(createAbird());
		}

	if (ShowBigBalloons == true) {
		var NUMBER_OF_BIG_BALLOON__IMAGES = 8; // images + 1
		var container8 = document.getElementById("big_balloonContainer");
		for (var i = 0; i < Number_Of_Big_Balloons; i++) { container8.appendChild(createABalloon()); }
	}

	if (ShowSmallBalloons == true) {
		var NUMBER_OF_SMALL_BALLOON__IMAGES = 6; // images + 1
		var container4 = document.getElementById("small_balloonContainer");
		for (var i = 0; i < Number_Of_Small_Balloons; i++) { container4.appendChild(createBBalloon()); }
	}
...
function createAmeteor() {
    ...
    meteorDiv.style.webkitAnimationName = "meteorfade, meteordrop" + meteorNumber;
...
function createACloud() {
    ...
    var image = document.createElement("img");
    image.src = "Images/Weather/fair_clouds/" + where + "/cloud" + randomInteger(1, NUMBER_OF_FAIR_CLOUD_IMAGES) + ".png";
...
function createAastronaut() {
    var image = document.createElement("img");
    image.id = "astronaut";	
    image.src = "Images/Weather/astronaut/astronaut1.png";
    return image;
}
